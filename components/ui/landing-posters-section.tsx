...
export function LandingPostersSection({
    ...
    // Reduce number of posters on mobile for better performance
    const visiblePosters = enableAnimations ? posters : posters.slice(0, 6)
    return (
        <section className={`relative py-24 px-6 bg-black overflow-hidden ${className}`}>
            ...
            <div className="relative z-10 max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-8 md:gap-12 max-w-7xl mx-auto">
                    {visiblePosters.map((posterNumber, index) => (
                        <div
                            key={posterNumber}
                            ...
                            style={{
                                ...
                                filter: enableHoverEffects ? 'drop-shadow(0 0 20px rgba(6,182,212,0.2))' : 'none'
                            }}
                            onClick={() => {
                        ...
                        >
                            {/* Glow border effect - only on desktop */}
                            {enableHoverEffects && (
                            ...
                            {/* Poster number badge - always visible on mobile */}
                            <div className={`absolute top-3 left-3 bg-gradient-to-r from-cyan-500 to-purple-500 text-white text-sm font-bold px-3 py-1 rounded-full z-20 ${enableHoverEffects ? 'opacity-0 group-hover:opacity-100 transition-all duration-300 transform -translate-y-2 group-hover:translate-y-0' : 'opacity-100'}`}>
                                {eventNames[posterNumber - 1]}
                            </div>

                            {/* Hover overlay with event info - only on desktop */}
                            {enableHoverEffects && (
                            ...
                            <Image
                                ...
                                loading={posterNumber <= 4 ? "eager" : "lazy"}
                            />
                ...
                hover:shadow-cyan-400/50">
                            ...
                            <button className="text-cyan-400 font-mono text-lg font-bold tracking-wider relative group overflow-hidden
                px-8 py-4 rounded-lg
                bg-gradient-to-r from-purple-600/20 to-cyan-600/20
                border border-cyan-400/30
                backdrop-blur-sm
